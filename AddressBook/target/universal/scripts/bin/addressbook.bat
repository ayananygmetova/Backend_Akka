@REM addressbook launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM ADDRESSBOOK_config.txt found in the ADDRESSBOOK_HOME.
...
if "%ADDRESSBOOK_HOME%"=="" (
  ...
  set "ADDRESSBOOK_HOME=%~dp0\\.."
) else (
  set "APP_HOME=%ADDRESSBOOK_HOME%"
)
...
set "CFG_FILE=%APP_HOME%\ADDRESSBOOK_config.txt"
set CFG_OPTS=
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running addressbook.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !ADDRESSBOOK_OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
