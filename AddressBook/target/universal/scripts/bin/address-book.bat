@REM address-book launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM ADDRESS_BOOK_config.txt found in the ADDRESS_BOOK_HOME.
@setlocal enabledelayedexpansion

@echo off


if "%ADDRESS_BOOK_HOME%"=="" (
  set "APP_HOME=%~dp0\\.."

  rem Also set the old env name for backwards compatibility
  set "ADDRESS_BOOK_HOME=%~dp0\\.."
) else (
  set "APP_HOME=%ADDRESS_BOOK_HOME%"
)
...
set "CFG_FILE=%APP_HOME%\ADDRESS_BOOK_config.txt"
set CFG_OPTS=
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running address-book.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !ADDRESS_BOOK_OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
